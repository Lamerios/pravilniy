...
const correctScoreSchema = z.object({
  ...
  reason: z.string().min(1, 'Необходимо указать причину исправления').max(500, 'Причина не может превышать 500 символов')
});
...
const scoreQuerySchema = z.object({
  ...
  gameId: z.string().optional().transform(val => val ? parseInt(val) : undefined),
  teamId: z.string().optional().transform(val => val ? parseInt(val) : undefined),
  roundId: z.string().optional().transform(val => val ? parseInt(val) : undefined)
});
 ...
 */
export const checkScoreAccess = (req: AuthenticatedRequest, res: Response, next: NextFunction): void => {
  try {
    const user = req.user;
...
export const validateGameScoresQuery = (req: Request, res: Response, next: NextFunction): void => {
  try {
    ...
    const querySchema = z.object({
      teamId: z.string().transform(val => parseInt(val)).refine(val => !isNaN(val) && val > 0).optional(),
      roundId: z.string().transform(val => parseInt(val)).refine(val => !isNaN(val) && val > 0).optional(),
      page: z.string().transform(val => parseInt(val)).refine(val => !isNaN(val) && val > 0).optional(),
      limit: z.string().transform(val => parseInt(val)).refine(val => !isNaN(val) && val > 0 && val <= 100).optional(),
      sortBy: z.enum(['createdAt', 'updatedAt', 'points', 'totalPoints', 'teamId', 'roundId']).optional(),
      sortOrder: z.enum(['ASC', 'DESC']).optional()
    });
 ...
 */
export const validateRoundScoresParams = (req: Request, res: Response, next: NextFunction): void => {
  try {
    const gameId = parseInt(req.params['gameId']!);
