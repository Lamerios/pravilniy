...
/**
 * Демонстрационный сидер для создания полного примера игры
 * Создает завершенную игру с командами, раундами и результатами
 */
export async function seedDemoGame(): Promise<void> {
  try {
    ...
    const adminUser = await User.findOne({ where: { email: 'admin@quiz-game.com' } });
    if (!adminUser) {
    ...
    // Создаем демонстрационную игру
    const demoGame = await Game.create({
      ...
      currentRound: 3,
      totalRounds: 3,
      startedAt: new Date(Date.now() - 7200000), // 2 часа назад
      finishedAt: new Date(Date.now() - 3600000), // 1 час назад
      isPublic: true,
      allowLateJoin: false,
      autoStart: false,
      settings: {
    ...
    // Создаем баллы за раунды (администратор вносит баллы для каждой команды за каждый раунд)
    const scores = await Score.bulkCreate([
      // Команда "Знатоки" - 1 место (85 баллов)
      {
        gameId: demoGame.id,
        teamId: teams[0]!.id,
        roundId: rounds[0]!.id,
