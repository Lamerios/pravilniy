...
async function main() {
    ...
    try {
        ...
        if (!forceClean) {
            ...
            console.log('');
            const confirmed = await (0, utils_1.waitForConfirmation)('Are you sure you want to clean all data?');
            if (!confirmed) {
        ...
        const beforeCounts = await (0, utils_1.getTableCounts)();
        const totalRecordsBefore = Object.values(beforeCounts).reduce((sum, count) => sum + (count > 0 ? count : 0), 0);
        ...
        await (0, utils_1.cleanAllTables)();
        console.log('\n📊 Step 4: Verifying cleanup...');
        const afterCounts = await (0, utils_1.getTableCounts)();
        const totalRecordsAfter = Object.values(afterCounts).reduce((sum, count) => sum + (count > 0 ? count : 0), 0);
